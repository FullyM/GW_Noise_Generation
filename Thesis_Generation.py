...
train_loader, val_loader, test_loader = construct_dataloaders('./Data/samples.h5', train_batch_size=batch_train,
                                                              val_batch_size=256, test_batch_size=256,
                                                              transform=torchvision.transforms.ToTensor(),
                                                              num_workers=17, pin_memory=True)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
writer = SummaryWriter(log_dir='./runs/first_VAE')

model = Noise_Models.FirstVAE().to(device)
...
fig1 = plt.figure()
plt.plot(train_loss)
plt.plot(val_loss, c='green')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend(['Train Loss', 'Validation Loss'])
plt.savefig('./ConvVAE/Loss_first')

summary(model, input_size=(batch_train, 3, 128, 128))
